    ...
    if (!db.io) {
      ...
      ioRoot.innerHTML = `
        <div class="section" style="background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%);border:1px solid #bfdbfe;padding:12px 16px">
          <div style="font-size:13px;font-weight:700;color:#1e40af">📦 Offline Mode - I/O Report Available (Client Engine)</div>
          <div style="font-size:11px;color:#475569;margin-top:2px">Current offline pack includes Gated/Ungated/CTB data. I/O Report can be generated by uploading 3 files or combined file directly in browser, no server needed.</div>
        </div>
      ` + (await (async()=>{
        return '';
      })());
      ...
      setTimeout(()=>{
        ...
        if (upSec && upSec.parentNode){
          ...
          banner.innerHTML=`<div style="font-size:13px;font-weight:700;color:#1e40af">📦 Offline Mode - I/O Report Available (Client Engine)</div><div style="font-size:11px;color:#475569;margin-top:2px">Built-in browser I/O engine: upload 3 files or combined xlsx to generate 9 reports. Supports Shift/Day/Week/Month, Line/ITEM/STYLE dims, merge, export Excel.</div>`;
