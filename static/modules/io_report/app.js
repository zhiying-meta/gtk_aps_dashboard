/**
 * IO and BOH - Optimized: Left Group Boxes + Dim-first + Lazy Expand + Paging
 */
(() => {
    ...
    if (!db.io) {
      ...
      ioRoot.innerHTML = `
        <div class="section" style="background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%);border:1px solid #bfdbfe;padding:12px 16px">
          <div style="font-size:13px;font-weight:700;color:#1e40af">📦 Offline Mode - IO and BOH Available (Client Engine)</div>
          <div style="font-size:11px;color:#475569;margin-top:2px">Current offline pack includes Gated/Ungated/CTB data. IO and BOH can be generated by uploading 3 files or combined file directly in browser, no server needed.</div>
        </div>
      ` + (await (async()=>{
        return '';
      })());
      ...
      setTimeout(()=>{
        ...
        if (upSec && upSec.parentNode){
          ...
          banner.innerHTML=`<div style="font-size:13px;font-weight:700;color:#1e40af">📦 Offline Mode - IO and BOH Available (Client Engine)</div><div style="font-size:11px;color:#475569;margin-top:2px">Built-in browser IO and BOH engine: upload 3 files or combined xlsx to generate 9 reports. Supports Shift/Day/Week/Month, Line/ITEM/STYLE dims, merge, export Excel.</div>`;
...
function attachUploadLogic(isCompact){
  ...
  async function fetchDataFolders(){
    ...
    try{
      ...
      if(!resp.ok) throw new Error(j.error||'Failed to list');
...
function renderReportsPage(){
    ...
    <div class="section" id="io-main-section">
      <div class="section-header">
        <div style="display:flex;align-items:center;gap:12px">
          <span class="section-title">📈 IO and BOH</span>
        ...
        <div class="section-actions">
          ...
          <button id="io-clear-all" class="btn btn-sm btn-outline" style="border-color:#ef4444;color:#ef4444">🗑️ Clear (Not Ready)</button>
          <button id="io-download-static" class="btn btn-sm btn-outline">📥 Download Static HTML</button>
        </div>
      </div>

      <!-- View Tabs: IO and BOH | Inventory Dashboard (Fusion per user request) -->
      <div class="dim-tabs" id="io-view-tabs" style="margin-bottom:8px;border-bottom:2px solid #e2e8f0">
        <button class="dim-tab active" data-view="reports">📈 IO and BOHs (9 Types)</button>
        <button class="dim-tab" data-view="inventory">📊 Inventory Dashboard — Balance Traceability <span style="font-size:9px;background:#dcfce7;color:#065f46;padding:1px 5px;border-radius:8px;margin-left:4px">NEW · Balance Pre-calculated</span></button>
      </div>

      <!-- IO and BOH Reports View -->
      <div id="io-view-reports">
      ...
      <!-- Inventory Dashboard View (Fused) - Style consistent with IO and BOH -->
      <div id="io-view-inventory" style="display:none">
        <div class="toolbar" style="border:none;padding:8px 0;margin-bottom:8px">
          <div class="panels-row" style="display:flex;gap:12px;align-items:stretch;flex-wrap:wrap">
            <div class="panel" style="flex:1;min-width:280px;background:#fff;border:1px solid #e2e8f0;border-radius:6px;padding:8px 12px">
              <div class="panel-label" style="font-size:11px;font-weight:600;color:#475569;text-transform:uppercase;margin-bottom:6px">📊 Inventory — Balance pre-calculated, display only</div>
              <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:6px">
                <span id="ioInvStatusBadge" style="font-size:11px;background:#f1f5f9;color:#64748b;border:1px solid #e2e8f0;padding:2px 8px;border-radius:12px">Loading...</span>
                <span style="font-size:11px;color:#64748b">One-button simultaneous rendering: shared balance data, no duplicate calculation</span>
              </div>
            </div>
            <div class="panel" style="min-width:240px;background:#fff;border:1px solid #e2e8f0;border-radius:6px;padding:8px 12px">
              ...
              <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
                <button id="ioInvBtnBuild" class="btn btn-sm" style="background:#0f172a;color:#fff;border-color:#0f172a">⚡ Build from IO and BOH Cache</button>
                <button id="ioInvBtnClear" class="btn btn-sm btn-outline" style="border-color:#ef4444;color:#ef4444">🗑️ Clear Inventory</button>
              </div>
            </div>
          </div>
        </div>
        <div id="ioInvContent" style="min-height:200px">
          <div style="text-align:center;padding:30px;color:#94a3b8">Loading Inventory Dashboard... (from IO and BOH cache + auto BOM)</div>
        </div>
      </div>
    </div>
  `;
  ...
  initMergeDragDrop();
  initIOViewTabs();
  // Auto-build inventory from IO and BOH cache after render (one-button simultaneous rendering per user request)
  setTimeout(() => { try { buildIOInventoryFromCache(); } catch(e) { console.warn('auto inventory build failed', e); } }, 500);
}

function initIOViewTabs() {
  const tabs = document.querySelectorAll('#io-view-tabs .dim-tab');
  const reportsView = document.getElementById('io-view-reports');
  const invView = document.getElementById('io-view-inventory');
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (view === 'reports') {
        ...
        // Lazy load inventory if not yet loaded
        if (window._ioInvData == null) {
          buildIOInventoryFromCache();
        } else {
          renderIOInventory();
        }
      }
      try { localStorage.setItem('io_currentView', view); } catch(e) {}
    });
  });
  // Restore last view
  try {
    const lastView = localStorage.getItem('io_currentView');
    if (lastView === 'inventory') {
      const invTab = document.querySelector('#io-view-tabs .dim-tab[data-view="inventory"]');
      if (invTab) invTab.click();
    }
  } catch(e) {}
  // Bind extra buttons
  document.getElementById('ioInvBtnBuild')?.addEventListener('click', () => buildIOInventoryFromCache(true));
  document.getElementById('ioInvBtnClear')?.addEventListener('click', async () => {
    ...
    try {
      ...
      _ioInvData = null;
      _ioInvExpanded = new Set();
      const badge = document.getElementById('ioInvStatusBadge');
      if (badge) badge.innerHTML = '<span style="background:#fef2f2;color:#991b1b;border:1px solid #fecaca;padding:2px 8px;border-radius:12px;font-size:11px">Cleared — Not Ready</span>';
      const content = document.getElementById('ioInvContent');
      if (content) content.innerHTML = '<div style="text-align:center;padding:30px;color:#94a3b8">Inventory cleared — click Build from IO and BOH Cache to rebuild</div>';
    } catch(e) { alert('Clear failed: '+e.message); }
  });
}

// ---------- Inventory inside IO and BOH ----------
let _ioInvData = null;
let _ioInvExpanded = new Set();
let _ioInvSelected = [];
let _ioInvSearch = "";
let _ioInvOnlyNeg = false;

async function buildIOInventoryFromCache(force = false) {
  const badge = document.getElementById('ioInvStatusBadge');
  const content = document.getElementById('ioInvContent');
    if (badge) badge.innerHTML = '<span style="background:#fef3c7;color:#92400e;border:1px solid #fde68a;padding:2px 8px;border-radius:12px;font-size:11px">⏳ Building from IO and BOH cache + auto BOM...</span>';
  if (content && !force && _ioInvData) {
    // Already have data, just render if needed
  }
  try {
    // First try to get existing built data
    let resp = await fetch('/api/inventory-dashboard/data');
    if (!resp.ok) {
      // Try to build
      const buildResp = await fetch('/api/inventory-dashboard/build', { method: 'POST' });
      const buildText = await buildResp.text();
      let buildJson;
      try { buildJson = JSON.parse(buildText); } catch { throw new Error('Build returned non-JSON: ' + buildText.slice(0,300)); }
      if (!buildResp.ok) throw new Error(buildJson.error || 'Build failed');
      _ioInvData = buildJson.dashboard || buildJson;
    } else {
      _ioInvData = await resp.json();
    }
    if (!_ioInvData || !_ioInvData.timeBuckets) throw new Error('No inventory data');
    if (badge) badge.innerHTML = '<span style="background:#dcfce7;color:#065f46;border:1px solid #86efac;padding:2px 8px;border-radius:12px;font-size:11px">✅ Ready: ' + Object.keys(_ioInvData.inventory||{}).length + ' mats × ' + (_ioInvData.timeBuckets||[]).length + ' buckets</span>';
    renderIOInventory();
  } catch (e) {
    console.error('buildIOInventoryFromCache failed', e);
    if (badge) badge.innerHTML = '<span style="background:#fef2f2;color:#991b1b;border:1px solid #fecaca;padding:2px 8px;border-radius:12px;font-size:11px">❌ ' + (e.message||'Failed') + '</span>';
    if (content) content.innerHTML = '<div style="text-align:center;padding:30px;color:#991b1b;background:#fef2f2;border:1px solid #fecaca;border-radius:6px">❌ Inventory build failed: ' + (e.message||'unknown') + '<br><span style="font-size:11px">Please load IO and BOH first and ensure BOM_Snapshot.xlsx exists in data/ (auto-scanned). Or go to standalone Inventory Dashboard Tab to upload dashboard_data.json</span></div>';
  }
}

function renderIOInventory() {
  const content = document.getElementById('ioInvContent');
  const renderer = window.InventoryRenderer;
  if (!content) return;
  if (!renderer) {
    content.innerHTML = '<div style="text-align:center;padding:20px;color:#991b1b">❌ InventoryRenderer not loaded — ensure renderer.js is included before io_report/app.js</div>';
    return;
  }
  if (!_ioInvData) {
    content.innerHTML = '<div style="text-align:center;padding:30px;color:#94a3b8">No inventory data — click Build button or load IO and BOH first</div>';
    return;
  }
  const state = { expanded: _ioInvExpanded, selectedSKUs: _ioInvSelected, searchText: _ioInvSearch, showOnlyNegative: _ioInvOnlyNeg };
  const callbacks = {
    onToggle: (pn) => { if (_ioInvExpanded.has(pn)) _ioInvExpanded.delete(pn); else _ioInvExpanded.add(pn); renderIOInventory(); },
    onExpandAll: () => { Object.keys(_ioInvData.bomChildren||{}).forEach(k=>_ioInvExpanded.add(k)); renderIOInventory(); },
    onCollapseAll: () => { _ioInvExpanded = new Set(); renderIOInventory(); },
    onSearch: (text) => { _ioInvSearch = text; clearTimeout(window._ioInvSearchTimer); window._ioInvSearchTimer = setTimeout(()=>renderIOInventory(), 250); },
    onSKUFilter: (selected) => { _ioInvSelected = selected; renderIOInventory(); },
    onClearFilter: () => { _ioInvSelected=[]; _ioInvSearch=""; _ioInvOnlyNeg=false; _ioInvExpanded=new Set(); renderIOInventory(); },
    onOnlyNegToggle: (checked) => { _ioInvOnlyNeg = checked; renderIOInventory(); },
    onExport: async () => {
      try {
  ...
  renderer.renderTable(content, _ioInvData, state, callbacks, 'ioInv');
  // Also expose for debugging
  try { window._ioInvData = _ioInvData; } catch(e) {}
}
...
function initReportsPage(){
  ...
  // Default to ITEM_NO if still empty to show table immediately (balance has data for ITEM_NO, but not LINE_CODE)
  if(dimOrder.length===0){
    dimOrder = ['ITEM_NO'];
    try{ localStorage.setItem('io_dimOrder', JSON.stringify(dimOrder)); }catch{}
  }
...
<title>IO and BOH - Static BI - ${now}</title>
<style>
...
<h1>📈 IO and BOH — Static BI Report (All Tabs & Flexible Dims)</h1>
...
<div style="margin-top:12px;font-size:10px;color:#94a3b8">Static BI report from IO and BOH dashboard. All tabs (FG/GB/FR/LT/RT) and flexible dims (Line/PN/Style, Column Shift/Day/Week/Month) preserved with embedded numbers. Filtering works offline. Open directly and share.</div>
<script>
